...
if __name__ == "__main__":
    ...
    metadata = []
    data_name_list = ["culture", "nature", "space"]
    for data_name in data_name_list:
        metadata_i = json.load(open(os.path.join(args.data_path, data_name + ".json"), "r"))
    ...
    print(args.model_fine_tuned_path)
    outdir = "./bagel_wise_self_reflection_v1_mr_ckpt_3000"
    ...
    for dataItem in tqdm(splited_metadata):
        ...
        if sample[2]:
            ...
            sample[0].save(os.path.join(sample_path, f"{prompt_id}.png"))

            with open(os.path.join(sample_path, f"{prompt_id}_refine.txt"), "w") as f:
                f.write(modify_text)
        else:
            image.save(os.path.join(sample_path, f"{prompt_id}.png"))
        image.save(os.path.join(sample_path, f"{prompt_id}_ori.png"))
