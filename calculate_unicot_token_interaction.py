...
@dataclass
class TokenInteractionStats:
    """Per-stage theoretical upper bound stats for token interactions (causal + KV-cache)."""
    stage: str  # e.g., "think", "breakdown", "generate", "eval", "refine", "editing"

    # Context composition
    prompt_text_len: int      # number of text tokens in KV-cache before decoding
    img_token_len: int        # number of image tokens in KV-cache before decoding
    gen_len: int              # number of generated text tokens in this stage

    # Per-token history length (upper bound, past-only)
    max_history_text: int     # max_i L_text^(i)
    avg_history_text: float   # average L_text^(i) over all generated tokens
    max_history_total: int    # max_i L_total^(i)
    avg_history_total: float  # average L_total^(i)

    # Aggregated interaction counts
    total_interactions_text: int   # sum_i L_text^(i)
    total_interactions_total: int  # sum_i L_total^(i)
    
    # Aggregated interaction counts for long chain
    total_interactions_text_naive: int = 0   # sum_i L_text^(i) for long chain
    total_interactions_total_naive: int = 0  # sum_i L_total^(i) for long chain

GLOBAL_TOKEN_STATS: List[TokenInteractionStats] = []


def _kv_lens_to_int(kv_lens) -> int:
    """Utility to extract scalar KV-cache length from kv_lens[0].

    Supports both Python int and 0-d torch.Tensor.
    """
    v = kv_lens[0]
...
def _current_cycle_slice(stats: List[TokenInteractionStats]) -> List[TokenInteractionStats]:
    """
    Return the sub-list corresponding to the *current* cycle, i.e. the suffix
    after the last 'think' stage. If there is no 'think', return the whole list.
    """
    last_think_idx = -1
    ...
    if last_think_idx == -1:
        # No 'think' found: treat the whole list as the current cycle
        return stats
    else:
        # Everything after the last 'think' belongs to the current cycle
        return stats[last_think_idx + 1:]
    

def summarize_current_cycle_eval_refine_editing(
    stats: List[TokenInteractionStats],
) -> Dict[str, int]:
    """
    Aggregate token stats over all ('eval', 'refine', 'editing')*n, 'eval', 'refine'
    stages in the *current* cycle.

    The current cycle is defined as the suffix of `stats` after the last 'think'
    stage (the list may end with an incomplete cycle).

    Within that current cycle:
      - For all stages where stage ∈ {"eval", "refine", "editing"}:
          * Sum `prompt_text_len`
          * Sum `gen_len`
      - For every *consecutive* ('eval', 'refine') pair in this suffix:
          * Add one `img_token_len` (assumes shared image KV; if they differ,
            use the max of the two).

    Returns:
        {
          "prompt_text_len": total over selected stages in the current cycle,
          "gen_len": total over selected stages in the current cycle,
          "img_token_len": total image tokens (once per eval-refine pair),
          "total_tokens": sum of all three,
        }
    """
    cycle = _current_cycle_slice(stats)

    total_prompt = 0
    total_gen = 0
    total_img = 0

    # 1) Sum prompt_text_len and gen_len for eval/refine/editing stage in current cycle
    for s in cycle:
        if s.stage in {"eval", "refine", "editing"}:
            total_prompt += s.prompt_text_len
            total_gen += s.gen_len
            total_img += s.img_token_len

    return {
        "prompt_text_len": total_prompt,
        "gen_len": total_gen,
        "img_token_len": total_img,
        "total_tokens": total_prompt + total_gen + total_img,
    }


def log_token_interaction_stats(stage: str,
                                prompt_text_len: int,
                                img_token_len: int,
                                gen_len: int,
                                logger,) -> TokenInteractionStats:
    """Compute and log theoretical upper bound on token interactions for one decoding stage.

    L_text^(i)  = prompt_text_len + (i - 1)
    L_total^(i) = L_text^(i) + img_token_len
    """
    if gen_len <= 0:
        stats = TokenInteractionStats(
            stage=stage,
            prompt_text_len=prompt_text_len,
            img_token_len=img_token_len,
            gen_len=0,
            max_history_text=prompt_text_len,
            avg_history_text=float(prompt_text_len),
            max_history_total=prompt_text_len + img_token_len,
            avg_history_total=float(prompt_text_len + img_token_len),
            total_interactions_text=0,
            total_interactions_total=0,
        )
        GLOBAL_TOKEN_STATS.append(stats)
        print(
            f"[TokenStats][{stage}] gen_len=0, "
            f"prompt_text_len={prompt_text_len}, img_token_len={img_token_len}"
        )
        logger.info(
            f"[TokenStats][{stage}] gen_len=0, "
            f"prompt_text_len={prompt_text_len}, img_token_len={img_token_len}"
    ...
    base_text = prompt_text_len
    base_total = prompt_text_len + img_token_len

    # Per-token history lengths (past-only)
    max_history_text = base_text + (gen_len - 1)
    avg_history_text = base_text + (gen_len - 1) / 2.0

    max_history_total = base_total + (gen_len - 1)
    avg_history_total = base_total + (gen_len - 1) / 2.0

    # Sum_i L_text^(i) = gen_len * base_text + gen_len * (gen_len - 1) / 2
    total_interactions_text = gen_len * base_text + gen_len * (gen_len - 1) // 2
    # Sum_i L_image^(i) = gen_len * base_total + gen_len * (gen_len - 1)
    if img_token_len == 0:
        total_interactions_total = total_interactions_text
    else:
        total_interactions_total = (gen_len * base_total + gen_len * (gen_len - 1))
        
    # naive cot
    previous_tokens = summarize_current_cycle_eval_refine_editing(GLOBAL_TOKEN_STATS)
    if previous_tokens["gen_len"] > 0 and stage in ["eval", "refine", "editing"]:
        total_interactions_text_naive = gen_len * (base_text + previous_tokens["prompt_text_len"]) + gen_len * (gen_len - 1) // 2
        if img_token_len == 0 and previous_tokens["img_token_len"] == 0:
            total_interactions_total_naive = total_interactions_text_naive
        else:
            total_interactions_total_naive = (gen_len * (base_total + previous_tokens["total_tokens"]) + gen_len * (gen_len - 1))
    else:
        total_interactions_text_naive = total_interactions_text
        total_interactions_total_naive = total_interactions_total
    ...
    stats = TokenInteractionStats(
        ...
        img_token_len=img_token_len,
        gen_len=gen_len,
        max_history_text=max_history_text,
        avg_history_text=avg_history_text,
        max_history_total=max_history_total,
        avg_history_total=avg_history_total,
        total_interactions_text=total_interactions_text,
        total_interactions_total=total_interactions_total,
        total_interactions_text_naive=total_interactions_text_naive,
        total_interactions_total_naive=total_interactions_total_naive,
    )
    GLOBAL_TOKEN_STATS.append(stats)

    print(
        f"[TokenStats][{stage}] "
        f"prompt_text_len={prompt_text_len}, img_token_len={img_token_len}, gen_len={gen_len}, "
        f"avg_history_text={avg_history_text:.2f}, avg_history_total={avg_history_total:.2f}, "
        f"max_history_text={max_history_text}, max_history_total={max_history_total}, "
        f"total_interactions_total={total_interactions_total}, total_interactions_total_naive={total_interactions_total_naive}"
    )
    logger.info(
        f"[TokenStats][{stage}] "
        f"prompt_text_len={prompt_text_len}, img_token_len={img_token_len}, gen_len={gen_len}, "
...
def calculate_all_stages(
    ...
    # get all cycles slices(delimited by 'think' stages)
    cycles = []
    last_think_idx = -1
    for i, s in enumerate(stats):
        if s.stage == "think":
            if last_think_idx + 1 < i:
                cycles.append(stats[last_think_idx + 1:i])
            last_think_idx = i
    if last_think_idx + 1 < len(stats):
        cycles.append(stats[last_think_idx + 1:])

    total_gen_tokens = [sum(s.gen_len for s in cycle) for cycle in cycles]
    total_interactions = [sum(s.total_interactions_total for s in cycle) for cycle in cycles]
    total_interactions_naive = [sum(s.total_interactions_total_naive for s in cycle) for cycle in cycles]
    
    avg_gen_tokens = sum(total_gen_tokens) / len(cycles) if len(cycles) > 0 else 0
    avg_interactions = sum(total_interactions) / len(cycles) if len(cycles) > 0 else 0
    avg_interactions_naive = sum(total_interactions_naive) / len(cycles) if len(cycles) > 0 else 0
    
    return {
        "avg_gen_tokens_per_cycle": avg_gen_tokens,
        "avg_interactions_per_cycle": avg_interactions,
        "avg_interactions_naive_per_cycle": avg_interactions_naive,
    }
...
def generate_think_and_breakdown(
    prompt, resolution=1024, max_length=2048, device=None, logger=None
):
    ...
    # Text KV length after SYSTEM_PROMPT + user prompt
    prompt_text_len_think = _kv_lens_to_int(newlens)
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        # Number of generated tokens for the "think" stage
        gen_len_think = int(unpacked_latent.shape[0])
        log_token_interaction_stats(
            stage="think",
            prompt_text_len=prompt_text_len_think,
            img_token_len=0,
            gen_len=gen_len_think,
            logger=logger,
        )
    ...
    # Text KV length after adding think_output
    prompt_text_len_breakdown = _kv_lens_to_int(newlens)
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        gen_len_breakdown = int(unpacked_latent.shape[0])
        log_token_interaction_stats(
            stage="breakdown",
            prompt_text_len=prompt_text_len_breakdown,
            img_token_len=0,
            gen_len=gen_len_breakdown,
            logger=logger,
        )
        output = tokenizer.decode(unpacked_latent[:,0])
...
def generate_evaluation_and_editing(
    image, prompt, think_text, resolution=1024, max_length=2048, device=None, max_image_size=1024, min_image_size=512, logger=None,
):
# whether the image is edited
    ...
    # Text-only KV length after SYSTEM_PROMPT + prompt + think_text
    text_only_len = _kv_lens_to_int(newlens)
    ...
    # KV length including image tokens
    total_len_with_image = _kv_lens_to_int(newlens)
    img_token_len = max(total_len_with_image - text_only_len, 0)
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        gen_len_eval = int(unpacked_latent.shape[0])
        log_token_interaction_stats(
            stage="eval",
            prompt_text_len=text_only_len,
            img_token_len=img_token_len,
            gen_len=gen_len_eval,
            logger=logger,
        )
        output = tokenizer.decode(unpacked_latent[:,0])
    ...
    # Text KV length after adding eval_output (still sharing the same image tokens)
    text_len_for_refine = _kv_lens_to_int(newlens) - img_token_len
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        gen_len_refine = int(unpacked_latent.shape[0])
        log_token_interaction_stats(
            stage="refine",
            prompt_text_len=text_len_for_refine,
            img_token_len=img_token_len,
            gen_len=gen_len_refine,
            logger=logger,
        )
        output = tokenizer.decode(unpacked_latent[:,0])
...
@torch.inference_mode()
def generate_image(prompt, num_timesteps=50, cfg_scale=4.0, cfg_interval=[0, 1.0], cfg_renorm_min=0., timestep_shift=1.0, resolution=1024, device=None, logger=None):
    past_key_values = NaiveCache(gen_model.config.llm_config.num_hidden_layers)
    newlens = [0]
    new_rope = [0]
    ...
    # Text KV length after prompt(think_text)
    prompt_text_len_think = _kv_lens_to_int(newlens)
    img_token_len = 4096  # 4096 -> 64 * 64
    ...
    with torch.no_grad():
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            ...
            gen_len_generate = int(unpacked_latent[0].shape[0])
            log_token_interaction_stats(
                stage="generate",
                prompt_text_len=prompt_text_len_think,
                img_token_len=img_token_len,
                gen_len=gen_len_generate,
                logger=logger,
            )
...
def editing_image(
    ...
    timestep_shift=3.0, max_image_size=1024, min_image_size=512, img_size=None, device=None, logger=None,
):
    # set output size
    if img_size is None:
        w, h = image.size
    ...
    img_token_len_vae = _kv_lens_to_int(newlens)
    print(f"Image token length from VAE: {img_token_len_vae}")
    ...
    img_token_len = _kv_lens_to_int(newlens)
    ...
    # Text KV length after adding prompt
    prompt_text_len = _kv_lens_to_int(newlens) - img_token_len
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        gen_len_editing = int(unpacked_latent[0].shape[0])
        log_token_interaction_stats(
            stage="editing",
            prompt_text_len=prompt_text_len,
            img_token_len=img_token_len,
            gen_len=gen_len_editing,
            logger=logger,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="output",
                        help='results path')
    parser.add_argument('--mode', type=str, default="pure_self_relfection",
    ...
    random.shuffle(splited_metadata)
    outdir = args.output_dir
    logger.add(args.log_path, rotation="10 MB")
    ...
    if args.mode == "pure_breakdown":
        ...
        for dataItem in tqdm(splited_metadata):
            ...
            while i < 20:
                ...
                if i < 10: # explicit calculate the token interaction between when reasoning_step=1~10
                    modify_flag = True
                else:
                    modify_flag = False
                ##### start editing
    ...
    # Global summary for all decoding stages in this run
    if GLOBAL_TOKEN_STATS:
        total_gen_tokens = sum(s.gen_len for s in GLOBAL_TOKEN_STATS)
        total_interactions = sum(s.total_interactions_total for s in GLOBAL_TOKEN_STATS)

        if total_gen_tokens > 0:
            avg_history_per_token = (
                sum(s.avg_history_total * s.gen_len for s in GLOBAL_TOKEN_STATS)
                / total_gen_tokens
            )
        else:
            avg_history_per_token = 0.0

        print(
            f"[TokenStats][GLOBAL] stages={len(GLOBAL_TOKEN_STATS)}, "
            f"total_generated_tokens={total_gen_tokens}, "
            f"avg_history_per_token={avg_history_per_token:.2f}, "
            f"total_interactions_upper_bound={total_interactions}"
        )
        print(1)

        summary_dict = calculate_all_stages(GLOBAL_TOKEN_STATS)
        print(f"{'*' * 20} Detailed Token Stats per Stage {'*' * 20}")
        for key, value in summary_dict.items():
            print(f"[TokenStats][{key}] {value}")
        # print in scientific notation
        print(f"{'='*20} Scientific Notation {'='*20}")
        for key, value in summary_dict.items():
            if isinstance(value, float):
                print(f"[TokenStats][{key}] {value:.2e}")
            else:
                print(f"[TokenStats][{key}] {value}")
